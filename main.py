...
class BlogPost(db.Model, Base):
    ...
    commentss = relationship("Comment", back_populates="blog_associated_to")
...
class Comment(db.Model, Base):
    ...
    blog_associated_to = relationship("BlogPost", back_populates="commentss")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment()
            new_comment.body = request.form["body"]
            new_comment.commenter = current_user
            new_comment.blog_associated_to = requested_post
            ...
            return redirect(url_for("login"))
    # comments = db.session.query(Comment).filter(Comment.blog_id == post_id).all
